...
const cleanUIToggle = document.getElementById("clean-ui-toggle");
    ...
    if (missed.length === 0) {
        ...
        for (const missedQuestion of missed) {
            ...
            if (missedQuestion.explanation) {
                ...
                explanationText.append(explanationLabel, document.createTextNode(missedQuestion.explanation));
...
function step() {
    ...
    renderScore();
    renderQuestion();
    engine.start((seconds) => (timerEl.textContent = `${seconds}s`), () => {
...
function handleAnswer(kind, value) {
...
muteToggle.addEventListener("change", () => soundEffect.setMuted(muteToggle.checked));
volumeSlider.addEventListener("input", () => soundEffect.setVolume(Number(volumeSlider.value)));
pauseResumeBtn.addEventListener("click", () => {
    if (engine.isPaused()) {
        soundEffect.play("resume");
        engine.resume((seconds) => (timerEl.textContent = `${seconds}s`), () => {
            soundEffect.play("timeout");
            engine.next();
            step();
        });
...
function onConfirm(btn, message, handler) {
...
function setCleanUI(on) {
    ...
    if (cleanUIToggle)
        cleanUIToggle.checked = on;
}
cleanUIToggle?.addEventListener("change", () => setCleanUI(cleanUIToggle.checked));
window.addEventListener("keydown", (e) => {
    ...
    const spacePressed = e.code === "Space" || e.key === " " || e.key === "Spacebar";
