...
export class SoundEngine {
  ...
  async play(cue: Cue) {
    this.ensureContext();
    if (!this.audioContext || !this.audioGain || this.muted) return;
    ...
    const set = (type: OscillatorType, audioFrequency: number) => {
    ...
    const trigger = (duration = 0.18, peak = 0.9) => {
      ...
      enviornemnt.gain.linearRampToValueAtTime(
        peak * this.masterVolume,
        now + 0.01
      );
